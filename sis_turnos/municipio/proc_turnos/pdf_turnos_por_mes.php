...
<h1 style="color:#242B5D; margin-top: 185px;">CLIENTES POR MES</h1>
    </div>
...
$consulta_mod = "SELECT cedula,nombre,apellido,turnos_proc.nroturno,turnos_proc.fecha,turnos.nombre_departamento,hora,turnos_proc.estado FROM turnos,turnos_proc,usuario,departamentos where
turnos.nroturno=turnos_proc.nroturno and turnos.idusuario=usuario.idusuario and turnos.nombre_departamento=departamentos.nombre_departamento and MONTH(fecha)='".$_GET['mes']."'  ;";
...
if (isset($_GET['fechas'])) {
    ...
    $date = date_create($fechas[0]);
    $fecha_inicio=date_format($date, 'Y-m-d');
    $date = date_create($fechas[1]);
    $fecha_fin=date_format($date, 'Y-m-d');
 ?>
<h1 style="color:#242B5D; margin-top: 185px;">CLIENTES POR FECHAS</h1>
    </div>
...
$consulta_mod = "SELECT cedula,nombre,apellido,turnos_proc.nroturno,turnos_proc.fecha,turnos.nombre_departamento,hora,turnos_proc.estado FROM turnos,turnos_proc,usuario,departamentos where
turnos.nroturno=turnos_proc.nroturno and turnos.idusuario=usuario.idusuario and turnos.nombre_departamento=departamentos.nombre_departamento and 
...
<?php
   $content = ob_get_clean();
    ...
    try
    {
        ...
        $html2pdf->Output('reporte_fechas_mes.pdf');
    }
